...
const run = async () => {
  ...
  assertCondition(
    result.recommendation.highlights.some((entry) =>
      entry.includes("Context signals: date")
    ),
    "Highlights should include context signals"
  );
  assertCondition(
    result.recommendation.highlights.some((entry) =>
      entry.includes("Formality mix: casual 0, smart-casual 1, formal 0")
    ),
    "Highlights should include formality mix"
  );
  assertCondition(
    result.recommendation.highlights.some((entry) =>
